...
overlay.style.cssText = `
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(0, 155, 90, 1); // Fundo verde
    z-index: 999;
`;

// Create a div for the green background behind the GIF
const greenBackground = document.createElement('div');
greenBackground.style.cssText = `
    position: absolute;
    top: 50%;
    left: 50%;
    ...
    background-color: rgba(0, 155, 90, 1); // Fundo verde
    z-index: 998; // Coloque atrás do GIF
`;
...
svgImage.style.cssText = `
    ...
    left: 50%;
    transform: translate(-50%, -50%);
    z-index: 999; // Coloque na frente do fundo verde
`;
svgImage.style.display = 'none';

// Append the elements to the body
document.body.appendChild(overlay);
document.body.appendChild(greenBackground);
document.body.appendChild(svgImage);

// Function to hide the overlay and display the SVG
function hideOverlay() {
    overlay.style.display = 'none';
    greenBackground.style.display = 'none'; // Esconder o fundo verde
